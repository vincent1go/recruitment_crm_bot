...
def найти_и_заменить_текст(страница, старый_текст, новый_текст, только_первые_n=0):
    ...
    if not области:
        # Пробуем найти с дополнительными символами (например, "Client: .")
        области = страница.search_for(старый_текст + " ")
        if not области:
            области = страница.search_for(старый_текст + ".")
            if not области:
    ...
    for область in области:
        # Смещаем текст вправо от метки "Client:" или "Date:"
        x_offset = область.x1 + 5  # Смещение вправо на 5 единиц от конца старого текста
        y_offset = область.y0 + 2  # Небольшое смещение вниз для выравнивания
...
def generate_pdf(путь_к_шаблону: str, текст: str, пользовательская_дата: datetime = None, выходная_папка: str = ".") -> str:
    ...
    for страница in документ:
        ...
        if это_imperative:
            ...
            # Для других шаблонов заменяем Date: (ограничиваем двумя вхождениями)
